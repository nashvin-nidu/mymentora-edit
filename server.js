...
import {
  createSegmentVideo,
  concatVideos,
  createVideoFromSegments,
} from "./lib/ffmpegHelpers.js";
...
app.use((err, req, res, next) => {
  if (
    ...
    if (req && req.rawBody) {
      ...
      snippet =
        req.rawBody.length > 2000
          ? req.rawBody.slice(0, 2000) + "...[truncated]"
          : req.rawBody;
    ...
    return res
      .status(400)
      .json({ error: "Invalid JSON", message: err.message });
  }
  next(err);
});
...
app.post("/generate-video", async (req, res) => {
  ...
  if (subtitlePreset) {
    ...
    globalSubtitleStyle =
      presets[subtitlePreset.toLowerCase()] || presets["default"];
  ...
  segments.forEach((s, i) =>
    console.log(`segment[${i}] imageUrl=${s.imageUrl} duration=${s.duration}`)
  );
  ...
  try {
    ...
    const downloadTasks = segments.map((seg, i) =>
      limit(async () => {
        ...
        await downloadFileToPath(seg.imageUrl, imgPath).catch((err) => {
          console.error(
            `Failed to download image for segment ${i}:`,
            err.message || err
          );
          throw err;
        });
    ...
    const subtitleStyles = subtitleResults.map(
      (result) => result.subtitleStyle
    );
    ...
    downloaded.forEach((seg, i) => {
      if (subtitleResults[i].calculatedDuration) {
        ...
        console.log(
          `Updated segment ${i} duration to ${seg.duration}s from word timing`
        );
      }
    });

    // Validate that each segment has a numeric duration (new payload requirement)
    const missingDuration = downloaded.findIndex(
      (s) => s.duration == null || !Number.isFinite(Number(s.duration))
    );
    if (missingDuration !== -1) {
      throw new Error(
        `Payload validation failed: each segment must include a numeric 'duration' in seconds (missing or invalid at index ${missingDuration})`
      );
    }
    ...
    try {
      ...
      console.error(
        "Single-run createVideoFromSegments failed, falling back to per-segment encode:",
